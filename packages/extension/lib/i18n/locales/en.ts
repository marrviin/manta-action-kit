...
export const en = {
  ...
  popup: {
    ...
    unsupportedPage: 'This page does not support the toolbar; use it on a regular web page',
  },
  home: {
    apiRecording: 'API Recording',
  ...
  detail: {
    ...
    error: 'Error',
    streamResponse: 'Stream Response · {{status}} {{statusText}} · {{count}} events',
  ...
  gateway: {
    ...
    logError: 'Error',
    curlCookieComment: 'Injected cookies (values not stored, names only): {{names}} ({{domain}})',
  },
  mcp: {
    connected: 'Connected',
    ...
    introDesc: "Manta Action Kit MCP opens the Chrome extension's capabilities to your Agent:",
    introBullet1:
    ...
    installPromptCopied: 'Install instructions copied — paste them to your Agent',
    status: 'Status',
    port: 'Bridge Port',
  ...
  mcpTools: {
    ...
    list_recordingsDesc: 'Get an overview of locally recorded API sets (name, origin, call count)',
    get_recordingLabel: 'Get recording detail',
    get_recordingDesc:
      "Get a recording's full call chain, including each API's request and response",
    ...
    get_callDesc: "Get one API call's request and response detail by its call ID",
