  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(STORE_GATEWAY_LOGS)) {
        const logs = db.createObjectStore(STORE_GATEWAY_LOGS, { keyPath: 'id' });
...
export async function saveRecording(recording: Recording, calls: ApiCall[]): Promise<void> {
...
export async function listRecordings(): Promise<Recording[]> {
  ...
  const all = await reqToPromise(
    tx(db, [STORE_RECORDINGS], 'readonly').objectStore(STORE_RECORDINGS).getAll() as IDBRequest<
      Recording[]
    >,
  );
...
export async function getRecording(id: string): Promise<Recording | undefined> {
  const db = await openDb();
  return reqToPromise(
    tx(db, [STORE_RECORDINGS], 'readonly').objectStore(STORE_RECORDINGS).get(id) as IDBRequest<
      Recording | undefined
    >,
  );
}
...
export async function getCalls(recordingId: string): Promise<ApiCall[]> {
  const db = await openDb();
  const index = tx(db, [STORE_CALLS], 'readonly').objectStore(STORE_CALLS).index('byRecording');
 ...
 */
export async function updateRecordingDeps(id: string, deps: FieldDependency[]): Promise<void> {
  const db = await openDb();
  const rec = await getRecording(id);
  if (!rec) return;
  rec.deps = deps;
 ...
 */
export async function updateRecordingDescription(id: string, description: string): Promise<void> {
  const db = await openDb();
  const rec = await getRecording(id);
  if (!rec) return;
...
/** Delete a recording and all of its calls. */
 ...
 */
export async function deleteCall(recordingId: string, callId: string): Promise<void> {
  const db = await openDb();
...
export async function listGatewayLogs(): Promise<GatewayLog[]> {
  const db = await openDb();
  const all = await reqToPromise(
    tx(db, [STORE_GATEWAY_LOGS], 'readonly').objectStore(STORE_GATEWAY_LOGS).getAll() as IDBRequest<
      GatewayLog[]
    >,
  );
...
export async function listGatewayProxyRules(): Promise<GatewayProxyRule[]> {
  ...
  for (const rule of all) {
    const isAgentSeed =
      rule.sandboxPrefix === '/proxytest' && rule.targetBase.includes('httpbin.org');
    if (isAgentSeed) {
...
export async function upsertGatewayProxyRule(rule: GatewayProxyRule): Promise<void> {
  const db = await openDb();
  await new Promise<void>((resolve, reject) => {
